    ...
    boardData=this.props.boardData.map((boardData, key) => {
      return (
        ...
        <div className="col-sm-4" style={{ padding: "7px", width: "100%" }} key={key}>
          <Card  onClick={() => this.handleOnclickCard(boardData.idboards)} body outline color="secondary" style={{ width: "80%", backgroundColor: "lightcyan" ,}} >
            <CardTitle style={{ fontWeight: " bolder", fontSize: "larger" }}>{boardData.bTitle}</CardTitle>
            <Button style={{ margin: "auto" }}  onClick={() => this.handleOnclickCard(boardData.idboards)}>Go to Board</Button>
          </Card>
        </div>
